...
This section provides a comprehensive guide to creating custom extensions for the Glueful framework v2.0, leveraging the modern service provider architecture and CLI tooling.
...
The main extension class must extend the `\Glueful\Extensions` class and implement both the `\Glueful\IExtensions` interface and optionally the `\Glueful\ExtensionInterface` for service provider functionality:
 ...
 * @author Your Name
 */
class YourExtension extends \Glueful\Extensions implements ExtensionInterface
{
    /** @var array Configuration for the extension */
    private static array $config = [];
    
    /**
     * Get service provider for DI container
     */
    public function getServiceProvider(): ?callable
    {
        return function (ContainerInterface $container) {
            // Register services with the DI container
            $container->singleton(YourService::class, function($c) {
            ...
            $container->singleton(YourController::class);
        };
    }
...
When you create an extension using the CLI, it's automatically registered in the `extensions.json` file. You just need to enable it:
...
If you created the extension manually, the system will detect it when you run:
...
The extension will be added to `extensions.json` with proper metadata:
...
```json
{
  "extensions": {
    "YourExtension": {
      ...
      "autoload": {
        "psr-4": {
          "Glueful\\Extensions\\YourExtension\\": "extensions/YourExtension/src/"
        }
      }
    }
  }
...
php glueful extensions install https://example.com/extension.zip ExtensionName

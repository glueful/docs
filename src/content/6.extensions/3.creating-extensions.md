...
return [
    ...
    'type' => 'optional',                   // Specify 'core' or 'optional'
...
When developing extensions for Glueful, it's important to properly indicate whether your extension should be treated as a core or optional component. This designation affects how your extension is managed by administrators and how it behaves in different environments.
...
Extensions should implement the `isEnabledForEnvironment()` method to control their activation in different environments:

```php
/**
 * Check if extension should be enabled in specific environment
 * 
 * @param string $environment The environment name (production, staging, development)
 * @return bool Whether the extension should be enabled
 */
public static function isEnabledForEnvironment(string $environment): bool
{
    if (self::getMetadata()['type'] === 'core') {
        return true; // Core extensions should be enabled in all environments
    }
...
### Dependency Management for Tiered Extensions

When specifying dependencies, be mindful of the tier implications:
...
{
    return [
        'UserManagement', // Core dependency
        'Analytics'      // Optional dependency
    ];
}
```

**Important**: If your extension depends on a core extension, your extension may also be treated as core by the system, especially if it's required for the core extension to function properly.

### Health Checks for Tiered Extensions

Core extensions should implement more robust health checks since their failure can impact the entire system:
...
public static function checkHealth(): array
{
    ...
    // More extensive checks for core extensions
    if (self::getMetadata()['type'] === 'core') {
        // Check critical dependencies
        if (!extension_loaded('mbstring')) {
            $issues[] = 'Required PHP extension "mbstring" is missing';
            $healthy = false;
        }
        ...
        // File system checks
        $requiredDirectories = ['storage/logs', 'storage/cache'];
        ...
        // Simpler checks for optional extensions
        // ...
    }
    ...
    return [
        ...
        'metrics' => [
            ...
            'execution_time' => 0, // Track with microtime() if needed
        ]
    ];
}
```

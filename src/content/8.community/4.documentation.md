...
From the security section (`/docs/src/content/4.security/.navigation.yml`):
  ...
  - label: Authentication
    ...
    children:
      - label: JWT Authentication
        to: /security/authentication/jwt
      - label: OAuth 2.0
        to: /security/authentication/oauth
      - label: Social Login
        to: /security/authentication/social-login
  - label: Authorization
    to: /security/authorization
  - label: Security Headers
    to: /security/headers
  - label: Rate Limiting
    to: /security/rate-limiting
```
...
8. **Version awareness**: Always indicate which version features were introduced or deprecated
9. **Test examples**: Ensure all code examples work with the documented version
...
Glueful documentation follows these versioning principles:

1. **Latest Version**: The main documentation always reflects the latest stable release
2. **Version Tags**: Major versions are tagged and preserved for reference
3. **Breaking Changes**: Clearly marked with version badges and migration guides
4. **API Versions**: API documentation includes version-specific information

### Version Indicators

Use these badges to indicate version-specific information:

```md
::badge{type="success"}
Available in v0.27.0+
::
...
1. Create a dedicated migration guide in `/docs/src/content/migration/`
2. Include:
...
For API endpoints, include version information:
...
GET /api/v2/users/{id}
```
Includes additional fields: `created_at`, `updated_at`
:::
::
```

For questions about documentation contributions, open a [GitHub Discussion](https://github.com/glueful/glueful/discussions) or review existing documentation issues.

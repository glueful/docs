...
::u-alert{type="info" title="Archiving moved to glueful/archive (1.52.0)"}
Table archiving is no longer a core capability. The framework no longer ships the archive schema, the `archive_registry` migrations, `config/archive.php`, or the `archive` capability gate (`ARCHIVE_DATABASE_SCHEMA`). To use archiving, `composer require glueful/archive` then `php glueful migrate:run` — the extension owns its own schema (namespace `Glueful\Extensions\Archive\*`).
::

::u-alert{type="info" title="Queue worker / job-metrics tables ship with glueful/queue-ops"}
The core `queue` capability above still owns `queue_jobs` / `queue_failed_jobs` / `queue_batches`. Supervised-fleet and job-metrics tables for autoscaling/supervision now ship with the **`glueful/queue-ops`** extension, not framework core.
::

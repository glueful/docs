...
app($context, EventService::class)->addListener(
    ...
    function (QueryExecutedEvent $event) {
        ...
        $this->analyticsService->trackDatabaseQuery([
            ...
            'tables' => $event->getMetadata('tables') ?? [],
            'execution_time' => $event->getExecutionTime(),
            'purpose' => $event->getMetadata('purpose')
        ]);
    }
);
```
...
$totalInserted = $db->transaction(function () use ($db, $users) {
    $count = 0;
    foreach ($users as $user) {
...
$result = $db->transaction(function () use ($db, $orderData, $inventoryUpdates) {
    // Create order (Glueful uses application-generated UUIDs, not auto-increment ids)
    $orderId = \Glueful\Helpers\Utils::generateNanoID();
    $db->table('orders')->insert(['uuid' => $orderId] + $orderData);

    // Create order items in a nested transaction (uses savepoints)
    $db->transaction(function () use ($db, $orderId, $orderData) {
    ...
    // Update inventory with a raw expression (raw() takes a single SQL string)
    foreach ($inventoryUpdates as $update) {
        $qty = (int) $update['quantity'];
        $db->table('inventory')
            ->where(['product_id' => $update['product_id']])
            ->update([
                'quantity' => $db->table('inventory')->raw("quantity - {$qty}")
            ]);
    }
...
Prefer the high-level `$query->transaction(fn(...) => ...)` API. If you need explicit control, use `TransactionManager`:
...
try {
    ...
    // Complex multi-step operation (insert() returns affected rows; use a
    // generated UUID for relationships)
    $userId = \Glueful\Helpers\Utils::generateNanoID();
    $db->table('users')->insert(['uuid' => $userId] + $userData);
    $db->table('profiles')->insert(array_merge($profileData, ['user_id' => $userId]));
...
$db->transaction(function () use ($db, $orderData) {
    $orderId = \Glueful\Helpers\Utils::generateNanoID();
    $db->table('orders')->insert(['uuid' => $orderId] + $orderData);

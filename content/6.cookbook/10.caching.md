...
7. **EdgeCacheService**: CDN and edge caching integration *(provided by the `glueful/cdn` extension; core ships only the no-op `NullEdgeCache` seam)*
...
- No pattern-based deletion: `deletePattern()` is not supported and returns false.
- No key enumeration: `getKeys()` returns an empty list and `getKeyCount()` returns 0.
- TTL lookup limited: `ttl()` cannot read remaining TTL natively and may be approximate or 0.
...
# Purge CDN by URL or tag (requires the glueful/cdn extension — not in core)
php glueful cache:purge --url="https://example.com/api/data"
php glueful cache:purge --tag=users
```

> `cache:purge` is provided by the **`glueful/cdn`** extension (framework 1.52.0+). Without it the command is not registered. See the [Edge Caching and CDN Integration](#edge-caching-and-cdn-integration) callout above.

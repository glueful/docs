...
description: Practical steps to deploy Glueful apps- env setup, Docker and server configs, health checks, production hardening, migrations, queues, and zero‑downtime tips.
---
...
- [ ] **Security hardening complete** (see [Security Hardening](/cookbook/security))
...
# Production readiness checks and cleanup
php glueful system:production --check --score
php glueful cache:clear

# Refresh the CLI command manifest (required after upgrading the framework or
# enabling/removing extensions — a stale storage/cache/glueful_commands_manifest.php
# referencing a removed command such as queue:autoscale, cache:purge, or archive:manage
# will break CLI boot).
...
**Image processing moved to `glueful/media` (framework 1.52.0+).** The `gd` / `freetype` /
`libjpeg` PHP build steps below are only needed if you use image processing, thumbnail
generation, or media metadata — which now ship in the optional
[`glueful/media`](https://github.com/glueful/media) extension rather than core. If your app
doesn't use media features, you can drop the GD build dependencies.
::
...
sudo apt install -y nginx mysql-server redis-server php8.3-fpm \
  ...
  php8.3-curl php8.3-zip php8.3-bcmath php8.3-json \
  php8.3-opcache certbot python3-certbot-nginx
...
# Configure PHP-FPM
sudo nano /etc/php/8.3/fpm/pool.d/glueful.conf
```
...
user = www-data
group = www-data
listen = /run/php/php8.3-fpm-glueful.sock
...
server {
    ...
    # PHP processing
    location ~ \.php$ {
        fastcgi_pass unix:/run/php/php8.3-fpm-glueful.sock;
...
# Refresh the CLI command manifest (stale manifests referencing removed commands
# break CLI boot after framework upgrades or extension changes)
echo "Refreshing command manifest..."
php glueful commands:cache --clear
...
# Run database migrations (APP_ENV is taken from the standby env's .env)
php glueful migrate:run --force
...
# Step 1: Preview pending migrations, then apply backward-compatible changes
...
# Step 3: Apply any remaining migrations after deployment
php glueful migrate:run --force
...
if [ -f "database_rollback_required" ]; then
    ...
    php glueful migrate:rollback --steps=$ROLLBACK_STEPS --force
fi

# Clear caches and refresh the command manifest
php glueful cache:clear
php glueful commands:cache --clear
...
restore_database_to_point_in_time() {
    ...
    # Apply any necessary data fixes (use a project-specific migration or script)
    php glueful migrate:run --force
...
opcache.fast_shutdown=0" >> /etc/php/8.3/fpm/conf.d/10-opcache.ini

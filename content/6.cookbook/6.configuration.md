...
description: Validate and normalize service options with Glueful’s internal SimpleOptionsResolver
---
...
Tip: For application configuration (files under `config/`), use the `config($context, 'key')` helper (or `$context->getConfig('key')`) inside factories/services. Use the options resolver for per‑service runtime options.

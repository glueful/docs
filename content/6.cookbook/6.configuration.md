---
title: Service Options Resolver
description: Validate and normalize per-service runtime options with Glueful’s internal SimpleOptionsResolver.
---

## Overview

Glueful provides an internal options resolver (`SimpleOptionsResolver`) for robust **per-service option validation** — type-safe defaults, validation, and normalization without requiring Symfony's OptionsResolver.

::u-alert{color="neutral" variant="subtle" icon="i-lucide-sliders-horizontal"}
This is **not** application configuration. For app config files under `config/` and environment variables, see [Advanced → Configuration](/advanced/configuration) and use the `config($context, 'key')` helper. Use the options resolver below for validating the options object a single service accepts at runtime.
::
...
::u-alert{color="neutral" variant="subtle" icon="i-lucide-info"}
This is an illustrative OptionsResolver example, not the shape of `config/queue.php`. Note
that worker-fleet keys like `workers.auto_scale`/`min_workers`/`max_workers` moved out of core
into the optional [`glueful/queue-ops`](https://github.com/glueful/queue-ops) extension
(`queue_ops.*`) in framework 1.52.0; core keeps the queue manager, drivers, and per-queue
`priority`/`memory_limit`/`timeout`/`max_jobs`.
::

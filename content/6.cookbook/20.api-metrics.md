---
title:   API Metrics
description: Async API performance and usage analytics with batching, daily aggregates, rate limit visibility, and system health endpoints.
---
...
3. [MetricsMiddleware Integration](#metricsmiddleware-integration)
...
2. **MetricsMiddleware**: `RouteMiddleware` for request interception
...
- **Glueful RouteMiddleware**: Native Glueful middleware (`RouteMiddleware`)
...
Note: Metrics recording is handled by `Glueful\Routing\Middleware\MetricsMiddleware`, which measures request duration, status code, and enqueues metrics via `ApiMetricsService::recordMetricAsync(...)` without impacting the request lifecycle.

...
description: Practical patterns for fast responses, query optimization, caching, profiling, and low-overhead analytics across Glueful’s stack.
---
...
// Store and retrieve reusable objects by key
if (!$pool->has('database_connection')) {
    $pool->add('database_connection', $this->createConnection());
}

$resource = $pool->get('database_connection');
$results = $resource->query($sql);

// Remove when no longer needed (or call clear() to empty the pool)
$pool->remove('database_connection');
```
...
6. [Chunked Database Processor](#chunked-database-processor)
7. [Lazy Container](#lazy-container)

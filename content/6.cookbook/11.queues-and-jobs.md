...
This comprehensive guide covers Glueful's enterprise-grade queue system, featuring multiple drivers, batch processing, and background job processing — plus supervised worker fleets and auto-scaling via the optional `glueful/queue-ops` extension.
...
- **Auto-Scaling** *(requires `glueful/queue-ops`)*: Intelligent worker scaling based on queue load and metrics
- **Process Management** *(requires `glueful/queue-ops`)*: Advanced worker process management with health monitoring
...
Core's `queue:work` runs a **single** worker. It takes no sub-actions — invoking one (e.g.
`queue:work spawn`) is now a command-not-found / "too many arguments" error.

```bash
# Start one worker (runs until stopped)
...
# Tune the worker (0 = unlimited for max-jobs / max-runtime)
php glueful queue:work --memory=512 --timeout=300 --max-jobs=100
```

### Supervise & Auto-Scale Workers

::u-alert{color="info" variant="subtle" icon="i-lucide-package"}
**Requires `glueful/queue-ops`.** The multi-worker supervisor and autoscaler commands below
ship in the [`glueful/queue-ops`](https://github.com/glueful/queue-ops) extension. The old
core `queue:work` sub-actions (`work`(multi)/`spawn`/`scale`/`status`/`stop`/`restart`/
`health`) were removed in framework 1.52.0; their functionality now lives under
`queue:supervise`.
::

```bash
# Supervise a fleet (spawns and monitors leaf workers)
php glueful queue:supervise --queue=emails --workers=4

# Worker status and management are handled by the supervisor

# Start the auto-scaling daemon
php glueful queue:autoscale

# Auto-scaler status and configuration
php glueful queue:autoscale status --detailed
php glueful queue:autoscale config --show
```

### Scheduler

```bash
# Run due scheduled jobs once
...
return [
    ...
    'workers' => [
        // Read by the lean core worker
        'performance' => [
            // backoff/sleep tuning consumed by queue:work
        ],

        // ⬇️ Moved to glueful/queue-ops as queue_ops.workers.* (framework 1.52.0+).
        // These keys no longer affect the core worker; they configure queue:supervise /
        // queue:autoscale once the extension is installed. The same env vars still apply.
        // 'process' => [ /* worker memory/timeout/max_jobs for supervised workers */ ],
        // 'auto_scaling' => [ /* enabled, limits, thresholds */ ],
        // 'resource_limits' => [ /* … */ ],
        // 'resource_thresholds' => [ /* … */ ],
        // 'supervisor' => [ /* … */ ],
    ],
...
::u-alert{color="info" variant="subtle" icon="i-lucide-package"}
**Per-queue `workers`/`max_workers`/`auto_scale`/`scale_*` keys require `glueful/queue-ops`.**
In framework 1.52.0 these moved to the extension's `queue_ops.*` namespace. Per-queue
`priority`, `memory_limit`, `timeout`, and `max_jobs` **stay in core**.
::
...
return [
    'queues' => [
        'emails' => [
            'auto_scale' => true,      // queue_ops.* (requires glueful/queue-ops)
            'min_workers' => 2,        // queue_ops.* (requires glueful/queue-ops)
            'max_workers' => 8,        // queue_ops.* (requires glueful/queue-ops)
            'scale_up_threshold' => 50,
...
# Auto-Scaling (consumed by glueful/queue-ops — queue_ops.*; no-op without the extension)
...
- **Lean Worker (core)**: A single-worker `queue:work` with `--once`/`--connection` support
...
- **Real-time Monitoring**: Queue statistics and performance analytics
- **Production Ready**: High-availability setup and performance optimization
- **Supervised Fleets & Auto-Scaling** *(via `glueful/queue-ops`)*: Worker supervision,
  intelligent scaling on real-time metrics, and persisted worker/job metrics

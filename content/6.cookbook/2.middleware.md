---
title: Middleware
description: Native RouteMiddleware, parameters, built‑ins, PSR‑15 and usage
---
...
- `admin`: Admin privilege checking with role-based access
- `field_selection`: GraphQL-style field selection for REST APIs
- `gate_permissions`: Gate/policy checks
- `auth_to_request`: Copy auth context to request attributes

Aliases are registered in `src/Container/Providers/CoreProvider.php`.

See the core concepts and API: **[Field Selection](/features/field-selection)**
...
- CORS handling with configurable policies (also supported by Router’s CORS logic)
...
use Glueful\Events\Http\{HttpAuthSuccessEvent, HttpAuthFailureEvent};
use Psr\Log\LoggerInterface;
...
$events->addListener(HttpAuthSuccessEvent::class, function (HttpAuthSuccessEvent $event) use ($logger) {
    // Log successful authentication
    $logger->info('User authenticated', [
        'user_id' => $event->tokenMetadata['user_id'] ?? null,
        'provider' => $event->tokenMetadata['provider'] ?? 'unknown',
        'path' => $event->request->getPathInfo()
    ]);
});

$events->addListener(HttpAuthFailureEvent::class, function (HttpAuthFailureEvent $event) use ($logger) {

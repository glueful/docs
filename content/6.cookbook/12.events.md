...
app($context, EventService::class)->dispatch(new UserRegisteredEvent(
    userId: $user['uuid'],
    email: $user['email'],
));
```
...
app($context, EventService::class)->addListener(
    ...
    function (UserRegisteredEvent $event) use ($context): void {
...
app($context, EventService::class)->addListener(
    ...
    [app($context, LogOrderPlacedListener::class), 'handle']
);
```

You can also pass a lazy, container-resolved reference as a string
(`'@'.LogOrderPlacedListener::class.':handle'`) so the listener is only
...
Register the subscriber (typically in a provider's `boot()`):

```php
use App\Listeners\UserLifecycleSubscriber;
use Glueful\Events\EventService;
...
app($context, EventService::class)->addListener(
    ...
    function (PasswordResetRequestedEvent $event) use ($context): void {

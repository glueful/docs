...
description: Standard API error shapes, framework ExceptionHandler, Response helpers, and practical patterns
---
...
Glueful uses a standardized error response format across all API endpoints, provided by Response helpers and the global ExceptionHandler.
...
| `error.type` | string | Optional machine-readable subtype (not always present) |
...
Response helpers:
- Response::validation(array $errors)
- Response::notFound(string $message = 'Resource not found')
- Response::unauthorized(string $message = 'Unauthorized')
- Response::forbidden(string $message = 'Forbidden')
- Response::rateLimited(string $message = 'Rate limit exceeded')
- Response::error(string $message, int $status = 400, mixed $details = null)
...
class ApiExceptionHandler
{
    ...
    public function handle(Throwable $exception): Response
    {
        if ($exception instanceof ValidationException) {
            return Response::validation($exception->errors());
        }
    ...
    private function handleValidationError(\App\Exceptions\ValidationException $e, string $requestId): array
    {
        return [
            'success' => false,
...
use App\Exceptions\{ValidationException, NotFoundException, DatabaseException};
use Glueful\Validation\Validator;
use Glueful\Validation\Rules\{Required, Length, Email};
...
class UserController
{
    ...
    {
        // Glueful's Validator takes a map of field => Rule[] objects.
        $validator = new Validator([
            'username'   => [new Required(), new Length(min: 3, max: 50)],
            'email'      => [new Required(), new Email()],
            'password'   => [new Required(), new Length(min: 8)],
            'first_name' => [new Required(), new Length(max: 100)],
            'last_name'  => [new Required(), new Length(max: 100)],
        ]);
        
        // validate() returns an errors map (field => string[]); empty means valid.
        $errors = $validator->validate($_POST);
        if ($errors !== []) {
            throw new ValidationException('Validation failed', $errors);
        }
        
        // filtered() returns the sanitized/normalized values.
...
// api/Exceptions/DatabaseException.php
...
use App\Exceptions\ValidationException;
// FileUploadException is defined in this file (same namespace) below.
...
/*
{
    "message": "Validation failed",
    "context": {
        "exception_type": "App\\Exceptions\\ValidationException",
        "file": "/var/www/glueful/api/Controllers/UserController.php",
        "line": 45,
        "stack_trace": "#0 /var/www/glueful/api/Controllers/UserController.php(45): App\\Validation\\UserValidator->validateRegistration()\n#1...",
        "request_data": {
            "method": "POST",
            "uri": "/api/v1/users",
            "query_string": "",
...
use App\Exceptions\{AuthenticationException, DatabaseException};
...
class ErrorHandlingTest extends TestCase
{
    ...
    {
        // Controllers extend BaseController and require ApplicationContext,
        // so resolve them from the container instead of instantiating directly.
        $controller = $this->get(UserController::class);
    ...
    public function testDatabaseErrorHandling()
    {
        ...
        // Resolve the controller from the booted app container.
        $controller = $this->get(UserController::class);
        $controller->create();
    }
}

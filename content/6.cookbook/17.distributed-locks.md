...
description: Distributed locking with Symfony Lock for safe concurrent processing
---
...
Use a lock to ensure only one supervised worker fleet runs per host/queue:

```php
// Prevents duplicate worker fleets for the same queue on this host
$this->lockManager->executeWithLock("queue:manager:{$queue}:" . gethostname(), function() {
    // ... start/supervise workers for $queue
}, 60);
```

::u-alert{color="neutral" variant="subtle" icon="i-lucide-info"}
Multi-worker supervision (`ProcessManager::scale()`) lives in the optional
[`glueful/queue-ops`](https://github.com/glueful/queue-ops) extension since framework 1.52.0.
Core's `queue:work` runs a single worker and does not coordinate a fleet.
::
...
```bash
# Run a single lean worker (core)
php glueful queue:work --queue=default

# Supervise/scale a worker fleet — requires the glueful/queue-ops extension
php glueful queue:supervise --workers=5 --queue=processing
```

...
description: Glueful’s Symfony Console-based CLI with DI-enabled commands for cache, database, extensions, queues, security, system checks, and development tools.
---
...
# Purge edge/CDN cache (requires the glueful/cdn extension — not in core)
php glueful cache:purge --url="https://example.com/api/data"
php glueful cache:purge --tag=users
```

::u-alert{type="warning" title="cache:purge requires glueful/cdn"}
The `cache:purge` command moved to the **`glueful/cdn`** extension in framework 1.52.0 and is **not registered** in core. Core keeps only the no-op `Glueful\Cache\NullEdgeCache` seam, so response caching still emits surrogate keys. Restore the command with `composer require glueful/cdn`.
::
...
```bash
# Enable extension (adds the provider FQCN to config/extensions.php 'enabled')
php glueful extensions:enable MyExtension

# Disable extension (removes it from 'enabled')
php glueful extensions:disable MyExtension

# Scaffold a new local extension (directory tree + ServiceProvider stub)
php glueful create:extension MyExtension
```

> Installing a published extension is a Composer operation
> (`composer require vendor/my-extension`), then `extensions:enable`. There are no
> `extensions:install`/`extensions:delete`/`extensions:validate` commands — discovery is
> Composer-only and activation is the single `enabled` allow-list.
...
Core ships a lean single worker (framework 1.52.0+):
...
# Pick the queue connection (0 = unlimited)
php glueful queue:work --connection=redis
```

::u-alert{type="warning" title="Supervised fleets / autoscaling require glueful/queue-ops"}
The old `queue:work` sub-actions (`work`/`spawn`/`scale`/`status`/`stop`/`restart`/`health`/`process`) and the `queue:autoscale` command were removed from core in framework 1.52.0. The **`glueful/queue-ops`** extension restores supervised, multi-worker fleets via `queue:supervise` + `queue:autoscale`. Restore with `composer require glueful/queue-ops`.
::

### Supervised Fleets & Auto-scaling (extension: glueful/queue-ops)

```bash
# Supervise a multi-worker fleet
...
For core concepts and API, see **[Field Selection](/features/field-selection)**
...
Validate and inspect database and Redis connection strings:

```bash
# Validate one or more database DSNs
php glueful config:dsn:validate --db="mysql://user:pass@localhost:3306/app"

# Validate Redis DSNs
php glueful config:dsn:validate --redis="redis://localhost:6379/0"

# Read DSNs from the environment (DATABASE_URL, REDIS_URL)
php glueful config:dsn:validate --from-env
```
